...
import (
	...
	//"github.com/anaray/fluent-bit-arrow-plugin/internal/dictutils"
...
type PayloadSchema struct {
	ArrowSchema Schema `json:"schema"`
}
...
func (f FieldWrapper) MarshalJSON() ([]byte, error) {
	fmt.Println("MarshalJSON calledd....")
...
func schemaToJSON(schema *arrow.Schema, mapper *Mapper) Schema {
	return Schema{
		Fields:    fieldsToJSON(schema.Fields(), NewFieldPos(), mapper),
		arrowMeta: schema.Metadata(),
	}
}

func SchemaFromJSON(schema Schema, memo *Memo) *arrow.Schema {
	sc := arrow.NewSchema(fieldsFromJSON(schema.Fields), &schema.arrowMeta)
	dictInfoFromJSONFields(schema.Fields, NewFieldPos(), memo)
	return sc
}

func dictInfoFromJSONFields(fields []FieldWrapper, pos FieldPos, memo *Memo) {
	for i, f := range fields {
		dictInfoFromJSON(f, pos.Child(int32(i)), memo)
	}
}

func dictInfoFromJSON(field FieldWrapper, pos FieldPos, memo *Memo) {
...
func fieldsToJSON(fields []arrow.Field, parentPos FieldPos, mapper *Mapper) []FieldWrapper {
...
func dictionariesFromJSON(mem memory.Allocator, dicts []Dictionary, memo *Memo) {
	for _, d := range dicts {
...
func recordsFromJSON(mem memory.Allocator, schema *arrow.Schema, recs []Record, memo *Memo) []arrow.Record {
...
func recordFromJSON(mem memory.Allocator, schema *arrow.Schema, rec Record, memo *Memo) arrow.Record {
	arrs := arraysFromJSON(mem, schema, rec.Columns)
	if err := ResolveDictionaries(memo, arrs, NewFieldPos(), mem); err != nil {
		panic(err)
	}
